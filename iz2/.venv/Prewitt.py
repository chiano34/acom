...
def prewitt_gradient(image):
    ...
    gradient_x = cv2.filter2D(image.astype(np.float32), -1, prewitt_x)
    gradient_y = cv2.filter2D(image.astype(np.float32), -1, prewitt_y)
...
def main(image_path,kernel,sigma,low,high):
    ...
    magnitude, direction = prewitt_gradient(blurred)
